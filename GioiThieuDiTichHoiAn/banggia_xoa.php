...
<?php
	...
	echo'<tr style="background-color: #d8d8d8"><th>Mã Số</th><th>Địa Điểm</th><th>Địa Chỉ</th><th>Thời Gian Mở Cửa</th><th>Giá Vé</th><th>Xóa</th><tr>';
	while ($row=mysql_fetch_array($rs,MYSQL_BOTH)) {
		echo'<tr><td>'.$row['MaDD'].'</td><td>'.$row['Diadiem'].'</td><td>'.$row['Diachi'].'</td><td>'.$row['Tgmocua'].'</td><td>'.$row['Giave'].'</td><td><a href="xoa.php?MDD='.$row['MaDD'].'">xóa</a></td></tr>';
	}
	echo'</table>';
